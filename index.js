...
while (num_1 != x && no_of_attempts <= 24) {
    ...
    if (num_1 == guess_num.inp) {
        console.log(`You have guessed the right number in ${no_of_attempts} attempt(s)`);
    }
    else {
        if (no_of_attempts > 15 && no_of_attempts < 25) {
            console.log(`Your desired number is between ${num_1 - 15} and ${num_1 + 15}`);
        }
        else {
            if (no_of_attempts == 25) {
